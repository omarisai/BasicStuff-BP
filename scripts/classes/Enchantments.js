import { EnchantmentList, MinecraftEnchantmentTypes, Enchantment, ItemStack, world } from '@minecraft/server';
		...
		for (let e in MinecraftEnchantmentTypes) {
			for (let e2 in MinecraftEnchantmentTypes[e]) {
				...
				if ((MinecraftEnchantmentTypes[e].id.toLowerCase()).includes(enchantName.toLowerCase())) {
					...
					if (!(enchantments.canAddEnchantment(new Enchantment(enchantType)))) throw `Enchantment ${enchantName} Incompatible with ${itemStack.typeId}!`;
